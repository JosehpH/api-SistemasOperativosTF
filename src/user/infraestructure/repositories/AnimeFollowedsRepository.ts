...
export class AnimeFollowedsRepository implements IAnimeFollowedsRepository {
  constructor(
    @InjectModel(AnimeFollowedEntity.name)
    private media: Model<AnimeFollowedEntity>,
  ) {}
    getUsersByAnimeId( idAnime: string) {
    return  this.media.find({ anime: {_id: idAnime} }).populate('anime').exec();
  }
   getAllAnimesByUserId(idUser: string) {
    return  this.media
      .find({ user: { _id: idUser } }).populate('anime').exec();
  ...
  async removeAnimeFromFolloweds(idAnime: string, idUser: string) {
    try {
      await this.media
        .deleteOne({ anime: { _id: idAnime }, user: { _id: idUser } })
        .exec();
      return true;
    } catch (error) {
      return false;
